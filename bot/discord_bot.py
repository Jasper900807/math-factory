...
async def cmd_status(ctx):
    ...
    # 最新影片
    latest = run_cmd(f"ls -t {WORK_DIR}/output/output_*.mp4 2>/dev/null | head -1")
...
async def cmd_start(ctx):
    if is_running("auto_producer.sh"):
...
async def cmd_stop(ctx):
    ...
    run_cmd("pkill -f 'python.*factory_v4'")
...
async def _watch_production(channel: discord.TextChannel, topic: str, log_path: str):
    """背景任務：輪詢生產 log，完成時推播通知。"""
    try:
    ...
    while asyncio.get_event_loop().time() < deadline:
        ...
        if "完成！最終檔案" in new_text or "[QUALITY]" in new_text:
            quality_match = next(
...
async def cmd_topic(ctx, *, topic: str):
    ...
    # 建立獨立 log 檔，避免混入 auto_producer log
    os.makedirs(f"{WORK_DIR}/logs", exist_ok=True)
    topic_log = f"{WORK_DIR}/logs/topic_{topic.replace(' ', '_')}.log"

    subprocess.Popen(
        [VENV_PYTHON, f"{WORK_DIR}/factory_v4.py", "--topic", topic],
        stdout=open(topic_log, "a"),
        stderr=subprocess.STDOUT,
    )

    await ctx.send(f"✅ 已觸發！生產完成後會自動通知")
    asyncio.create_task(_watch_production(ctx.channel, topic, topic_log))
